...
按"工程师对我们代码库零上下文且品味可疑"来写一份完整的实施计划。把他们需要知道的一切都写明白：每个任务要碰哪些文件、要看的相关代码和文档、怎么验证。把整份计划拆成细粒度任务交给他们。DRY。YAGNI。

假设他们是熟练开发者，但对我们的工具链或问题域几乎一无所知。

**默认不强制 TDD 或单元测试。** 测试由用户/任务显式要求时才加入；默认验证用项目已有的静态检查（类型检查 / lint / 构建）。**默认不自动 commit**——任务完成后只 stage（`git add`），由用户审阅并自行 commit。
...
**每个步骤是一个动作（2-5 分钟）。默认拆解形式：**
- "实现 `<具体函数/组件>`" - 一个步骤
- "运行项目的静态检查（类型检查 / lint / 构建）" - 一个步骤
- "暂存变更（`git add`），等用户审阅" - 一个步骤

**如果**任务明确要求 TDD 或测试覆盖，**才**采用红绿循环形式：
- "写失败的测试" - 一个步骤
- "运行它确保它失败" - 一个步骤
- "编写让测试通过的最小实现" - 一个步骤
- "跑测试确保通过" - 一个步骤
...
Run: `<项目实际命令，例如 mypy / tsc --noEmit / cargo check / pnpm build>`
Expected: 通过，无新错误
...
- 精确的命令和预期输出
- DRY、YAGNI
- 默认 stage 待用户审阅，**不要**自动 commit
- 默认不写单元测试；**任务明确要求**时才使用上面的 TDD 模板
